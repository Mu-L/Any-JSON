...
func to_json(value, ruleset:Dictionary) -> Dictionary[String,Variant]:
	...
	or value is PackedVector4Array or value is PackedColorArray:
...
func from_json(json:Dictionary, ruleset:Dictionary) -> Variant:
	...
	if value is not String && type != 'PackedStringArray':
		report_error(1)
		return null
	if type == 'PackedStringArray' && value is not Array:
		report_error(1)
		return null
	...
	elif type == 'PackedColorArray': return hex_to_bytes(value).to_color_array()
